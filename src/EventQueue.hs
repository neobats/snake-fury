...
{- | Given the current score and the event queue, updates the new speed and returns it.
   This action is mutable, therefore must be run in the IO monad
-}
...
setSpeed s (EventQueue _ m_current initial_speed) = do
  ...
  let new_speed = calculateSpeed s initial_speed -- calculate new speed based on the score. This is a pure calculation, hence the let at the beginning
...
{- | This function translates key strokes to movements and push them into the queue.
 The player is free to push keys as fast as he/she can but the userqueue is bounded,
